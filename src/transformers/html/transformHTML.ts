...
import { ICollectionData, ISection, IListTypes, IBlock, IBlockFields } from '../../types'

export const transformHTML = (data: ICollectionData): string => {
  return reduce(data.sections, (html: string, sectionVal: ISection[]): string => {
    let element: string
    ...
    sectionVal.forEach((block: IBlockFields): void => {
      switch (block.type) {
        case 'header':
